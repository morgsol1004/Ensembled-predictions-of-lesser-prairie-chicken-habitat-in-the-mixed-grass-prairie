...
#See ModelSelection.R file for breakdown of how these 9 top models were selected for AIC analysis
...
library(ResourceSelection)
hoslem.test(LPCHdata$PT_Type, fitted(Cand.models[[2]]))  #model appears to fit the data well because we have not significant difference between the model and observed data. Large p-values mean that the model fits the data well.
...
perf <- performance(preds,"tpr","fpr") #tpr = true positive rate, fpr = false positive rate
plot(perf)
abline(a=0, b=1)
...
#building raster stack to use predict function on below
stack<-stack(AveTree,AvePFG,AveAFG,AveShrub,DensOil,DensWind,DistHighway,DistWind,DistTrans,Ruggedness,VarPFG,VarShrub,VarBG,AveTemp,AveCrop) 
...
Suitability<-predict(stack, model2, filename = "RSF_AllLeks_Newdata.tif")
